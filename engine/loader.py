...
def _parse_champion(
    raw: Any, index: int, c: _Collector, role_mana: Mapping[str, float]
) -> ChampionDef | None:
    where = f"champions.json[{index}]"
    if not isinstance(raw, dict):
    ...
    if isinstance(champ_id, str) and champ_id:
        where = f"champions.json[{champ_id}]"
    else:
    ...
    stats = _parse_stats(where, raw.get("stats"), c, role, role_mana)
...
def _parse_stats(
    ...
    values["max_mana"] = c.number(f"{where}.stats", raw, "max_mana", minimum=1)
...
def _parse_combat_config(raw: Any, c: _Collector) -> CombatConfig | None:
    ...
    expected = set(_COMBAT_POSITIVE_KEYS) | set(_COMBAT_NON_NEGATIVE_KEYS) | {
        "damage_mana_roles"
    }
    c.require_keys(where, raw, expected, expected)
    ...
    return CombatConfig(damage_mana_roles=frozenset(roles_raw), **values)
...
def load_all(data_dir: Path | str = DEFAULT_DATA_DIR) -> GameData:
    ...
    log.info(
        "Loaded %d champions, %d traits, %d items (set %s, patch %s)",
        len(champions),
        len(traits),
        len(items),
        version.set,
