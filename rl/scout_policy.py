...
This is the representation contract from doc 99 entry 160, not evidence that
PPO can use the representation.  No training code selects it yet.
"""
...
class ScoutSetExtractor(BaseFeaturesExtractor):
    ...
    def __init__(
        ...
        required = {"global", "opponent_units", "opponent_board", "opponent_augments"}
        ...
        self.item_embedding = nn.Embedding(max_token_id, 12)
        ...
        self.output_net = nn.Sequential(
            nn.Linear(global_dim + board_dim, features_dim),
            nn.ReLU(),
        )
    ...
    def forward(self, observations: dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        return self.output_net(torch.cat((global_context, opponent_context), dim=-1))
