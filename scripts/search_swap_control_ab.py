"""Is the swap gain the simulation's judgement, or just the board churn?

Doc 99 entry 160.141. 160.140 measured one ``best_swap`` per phase at −0.450
against the established buy+item teacher on one block, and named the two things
that leaves open: the result has no replication, and it has no no-search floor.
This run supplies both at once. ``blind`` changes boards from the same
shortlist at the same measured rate while simulating nothing, so ``swap`` minus
``blind`` is the placement the *judgement* buys.
"""
...
def source_fingerprint() -> str:
    ...
    for name in (
        ...
        "rl/opponents.py",
        "rl/search.py",
        "scripts/search_swap_control_ab.py",
    ):
...
def summarise(records: list[dict]) -> tuple[dict, dict]:
    ...
    comparisons = {
        "swap minus base (replication)": paired(
            summary["base"]["placements"], summary["swap"]["placements"]
        ),
        "blind minus base (churn floor)": paired(
            summary["base"]["placements"], summary["blind"]["placements"]
        ),
        "swap minus blind (the judgement)": paired(
            summary["blind"]["placements"], summary["swap"]["placements"]
        ),
    }
...
def main() -> int:
    ...
    parser.add_argument("--seed0", type=int, default=79_000)
