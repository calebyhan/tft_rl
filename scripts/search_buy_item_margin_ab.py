"""Can buy's conservative acceptance rule repair buy-plus-item search?"""
...
def source_fingerprint() -> str:
    ...
    for name in (
        ...
        "rl/opponents.py",
        "rl/search.py",
        "scripts/search_buy_item_margin_ab.py",
    ):
...
def _policy(env: TFTEnv, arm: str, seed: int):
    ...
    return search_item_greedy_policy(
        ...
        panel_size=2,
        trials=2,
        margin=0.25 if arm == "margin" else 0.0,
        buy_search=True,
    )
...
def summarise(records: list[dict]) -> tuple[dict, dict]:
    ...
    comparisons = {
        ...
        "margin minus buy": paired(
            summary["buy"]["placements"], summary["margin"]["placements"]
        ),
        "margin minus combined": paired(
            summary["combined"]["placements"], summary["margin"]["placements"]
        ),
    }
...
def main() -> int:
    ...
    parser.add_argument("--seed0", type=int, default=76_000)
