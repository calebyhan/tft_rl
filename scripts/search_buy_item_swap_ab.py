"""Does exact one-swap fielding search add value to the buy+item teacher?

Doc 99 entry 160.139. The control is the policy 160.136 and 160.138 established
across 600 fresh paired seeds: faithful FAST8 ``GreedyActionPolicy``, exact
``best_buy`` first in phase, exact ``best_item_prefix`` at depth 1 with
acceptance margin 0.25 after the board settles. The two treatment arms add one
``best_swap`` decision per planning phase and differ **only** in where it fires
relative to the item phase, because items cannot be moved once equipped.
"""
...
ARMS = ("base", "swap_first", "swap_last")

# ``best_swap`` sums its candidate scores over the panel and averages over
# trials, exactly as ``best_buy`` does, and accepts on ``> baseline + margin``
# against ``best_buy``'s ``> baseline + margin * panel_size``.  At panel 2 the
# shipped 0.5 is therefore the same 0.25-per-opponent threshold the standing
# buy and item policies use.  Transferred, not tuned; trials drops from the
# shipped 3 to 2 so all three components share one estimator budget.
...
def source_fingerprint() -> str:
    ...
    for name in (
        ...
        "rl/opponents.py",
        "rl/search.py",
        "scripts/search_buy_item_swap_ab.py",
    ):
...
def _policy(env: TFTEnv, arm: str, seed: int):
    """The 160.136 teacher, optionally with one ``best_swap`` per phase."""
    return search_item_greedy_policy(
        ...
        swap_before_items=arm == "swap_first",
    )
...
def summarise(records: list[dict]) -> tuple[dict, dict]:
    ...
    comparisons = {
        "swap_first minus base": paired(
            summary["base"]["placements"], summary["swap_first"]["placements"]
        ),
        "swap_last minus base": paired(
            summary["base"]["placements"], summary["swap_last"]["placements"]
        ),
        "swap_first minus swap_last": paired(
            summary["swap_last"]["placements"], summary["swap_first"]["placements"]
        ),
    }
...
def main() -> int:
    ...
    parser.add_argument("--seed0", type=int, default=78_000)
